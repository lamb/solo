...
function buildIndex(){
  console.log('Build file: index.html');
  ...
  files.forEach(function(file){
    ...
    if (dotLastIndex >= 0) {
      ...
      if (fileType === 'md') {
          ...
          content = md(content);
          tmpldata.article.push(content);
      }
    }
  });
  //console.log(fs.readFileSync(indexTemplate,charset));
  //模板引擎
  content = tmpl(fs.readFileSync(indexTemplate,charset))(tmpldata);
...
function buildFile(file, fileName, from, to, template) {
  ...
  //模板引擎
  content = tmpl(content)(tmpldata.config);
